# -*- coding: utf-8 -*-
"""Omar Al-Gilani voice model  - RVC Training

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1kKQW2jHP-FluULVAtCoKEkuZM_yS0J2A

#**أداة أستنساخ وتغيير الصوت**
...
# ***الخطوة الرئيسية: تثبيت الإضافات***
...
---
#***المرحله الاولى***: **استنساخ الصوت**

**تدريب النموذج**
"""

#@title **الخطوة 1:** تحميل الصوت
#@markdown أضغط على تشغيل وبعدها قم بتحميل ملف صوتي نظيف واحد. **( الموصى به: ملف صوتي نظيف مدته من 3 إلى 7 دقائق)**
import os
...
# Commented out IPython magic to ensure Python compatibility.
#@title **الخطوة 2:** إنشاء ملفات التدريب
import os
...
# %cd /content/RVC
#@markdown  يجب أن يكون اسم النموذج باللغة الإنجليزية. لا تستخدم "المسافات" أو "الرموز".

#@markdown **يجب عليك استخدام نفس اسم النموذج في الخطوات التالية وإلا فستواجه أخطاء.**
...
#@markdown  أدخل اسم النموذج الخاص بك مرة أخرى. يجب أن يكون **نفس ما اخترته أعلاه**
model_name = ''#@param {type:"string"}
#@markdown ---

#@markdown  "القيمه" هي مقدار ما ترغب في تدريب نموذجك عليه. **(الموصى به: 200 - 1000)**
...
**نماذج النسخ الاحتياطي والتحميل**
"""

#@title **نسخ احتياطي للنموذج إلى جوجل درايف**

#@markdown  أدخل نفس الاسم الدقيق للنموذج الذي قمت بتدريبه.
...
#@title **أختياري:** تنزيل ملف النموذج

#@markdown  استخدم كتلة التعليمات البرمجية هذه لتنزيل ملف .pth الخاص بالنموذج لاستخدامه في أي مكان.
...
---
#***المرحله الثانيه***: **استبدال الصوت**"""

#@title **الخطوة 1:** تحميل النموذج من جوجل درايف


!pip install --upgrade --no-cache-dir gdown
import tarfile

#@markdown  أدخل نفس الاسم الدقيق للنموذج الذي تريد تحميله.

Model_Name = "sear"#@param {type:"string"}

#@markdown _قم بلصق رابط الملف من جوجل درايف وتذكر أن تجعله رابطًا عامًا._

model_pth_name = Model_Name + ".tar.gz"

MODEL_LINK = "https://drive.google.com/file/d/1-24rb1hnHlfUlkM9pkDJpkVMvw7Y7SSU/view?usp=drive_link" #@param {type:"string"}
...
#@title **الخطوة 2:** تحميل الصوت المستهدف
#@markdown يجب أن يكون ملف الصوت المستهدف عبارة عن صوت صوتي نظيف للغاية.
...
# Commented out IPython magic to ensure Python compatibility.
#@title **الخطوة 3:** تشغيل الاستدلال
import os
import IPython.display as ipd
# %cd /content/RVC

#@markdown  أدخل اسم النموذج الذي تريد استخدامه.
model_name = 'sear'#@param {type:"string"}
...
#@markdown ---
#@markdown **قم بتعيين القيم اللازمه للاستنساخ...**

#@markdown **ذكر لذكر أو أنثى لأنثى : 0**

#@markdown **من أنثى إلى ذكر : -12**

#@markdown **من ذكر إلى أنثى : 12**
...
#@title **الخطوة 4:** تنزيل الصوت الناتج
#@markdown * استخدم هذه الخلية لتنزيل ملف الصوت المحول النهائي.
#@markdown * إذا كان ملف الصوت النهائي أطول من 4 دقائق، فقد لا تظهر المعاينة في الخطوة السابقة. لكنه تم تحويله بالفعل ويمكنك تنزيله باستخدام هذه الخلية.
